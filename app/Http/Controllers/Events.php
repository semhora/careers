...
class Events extends Controller
{
    ...
    public function confirm(Request $request)
    {
        ...
        if( $fields['status'] == true ) {
            $db = \DB::table('events_confirmeds')
                ->where('event_id', $fields['event'])
    ...
    public function store(Request $request )
    {
        ...
        if( $fields ) {
            ...
            $event->categories = (isset($fields['categories']) ? json_encode( $fields['categories'] ) : json_encode([]));
    ...
    public function cancel( $id )
    {
        ...
        if( $event->save() ) {
            \App\Helpers\Events::setNotificationStatus( $event, 'closed' );
            return redirect('panel/events/update/' . $id);
        } else {
            session()->flash('error', 'Não foi possível cancelar o evento.');
            return redirect('panel/events/update/' . $id);
        }
    }
    ...
    public function active( $id )
    {
        ...
        if( $event->save() ) {
            \App\Helpers\Events::setNotificationStatus( $event, 'open' );
            return redirect('panel/events/update/' . $id);
        } else {
            session()->flash('error', 'Não foi possível ativar o evento.');
            return redirect('panel/events/update/' . $id);
        }
    }
}
