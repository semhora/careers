...
class Events
{
    ...
    {
        $user = \DB::table( 'events_confirmeds' )
...
//        krsort( $new );
    ...
    static public function getEventPerAge( $event )
    {
        ...
        if( isset($confirmeds) ) {
            ...
            foreach( $confirmeds as $id ) {
                ...
                if( isset($user->profile->fields->birth) )
                {
            ...
            return [
                'ages' => [
                    '0-20'  => (isset($ages['0-20']) ? count( $ages['0-20'] ) : 0),
                    '20-30' => (isset($ages['20-30']) ? count( $ages['20-30'] ) : 0),
                    '30-40' => (isset($ages['30-40']) ? count( $ages['30-40'] ) : 0),
                    '40>'   => (isset($ages['40>']) ? count( $ages['40>'] ) : 0),
                ],
                'genre' => [
                    'masc'  => (isset($genre['masc']) ? count( $genre['masc'] ) : 0),
                    'fem'   => (isset($genre['fem']) ? count( $genre['fem'] ) : 0),
                ]
            ];
        } else {
            return [
    ...
    static public function setNotificationStatus( $event, $status )
    {
        if( $event->total_confirmeds > 0 )
        {
            $when = Carbon::now()->addMinute(1);
            foreach( $event->confirmeds as $item )
            {
                $user = \App\Models\User::find($item);
                \Mail::to( $user->email )
                    ->later($when, new EventStatus( $event, $status ));
            }
        }
    }
}