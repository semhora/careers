...
class Events extends Model
{
    ...
    {
        $categories = json_decode( $this->attributes['categories'] );
        $newCategories = '';
        if( isset( $categories ) ) {
            foreach ($categories as $category) {
                $newCategories .= \App\Models\Category::find($category)->name . ', ';
            }
        }
        $newCategories .= ';;';

        return str_replace(', ;;', '', $newCategories);
    }
    ...
    {
        if(isset(\Auth::user()->profile->fields->cep)) {
